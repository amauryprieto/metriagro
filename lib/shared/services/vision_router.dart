...
class VisionRouter {
  ...
  VisionRouter({List<VisionClassifier>? classifiers, VisionClassifier? fallbackClassifier})
  ...
  Future<VisionResult> classify(Uint8List imageData, CropType? expectedCropType) async {
    print('[VisionRouter] Starting image classification with expectedCropType: $expectedCropType');
    print('[VisionRouter] Available classifiers: ${_classifiers.keys.toList()}');
    print('[VisionRouter] Fallback classifier available: ${_fallbackClassifier != null}');
    ...
    if (expectedCropType != null && _classifiers.containsKey(expectedCropType)) {
      ...
      print('[VisionRouter] Found classifier for $expectedCropType: ${selectedClassifier?.name}');

      if (selectedClassifier != null && await selectedClassifier.isReady()) {
        print('[VisionRouter] Using specific classifier for ${expectedCropType}');
        try {
          final result = await selectedClassifier.classify(imageData);
          print('[VisionRouter] Classification successful: ${result.diseaseName} (${result.confidence})');
          return result;
        } catch (e) {
          print('[VisionRouter] Error with specific classifier: $e');
          // Continue to try other classifiers
        }
      } else {
        print('[VisionRouter] Specific classifier not ready or null');
      }
    }

    // 2. Si no hay tipo específico, probar todos los clasificadores
    if (expectedCropType == null) {
      print('[VisionRouter] No specific crop type, trying all classifiers');
      for (final entry in _classifiers.entries) {
        final classifier = entry.value;
        print('[VisionRouter] Checking classifier for ${entry.key}: ${classifier.name}');

        if (await classifier.isReady()) {
          print('[VisionRouter] Trying classifier for ${entry.key}');
          try {
            final result = await classifier.classify(imageData);
            print('[VisionRouter] Classification result: ${result.diseaseName} (${result.confidence})');

            // Si la confianza es alta, usar este resultado
            if (result.confidence > 0.7) {
              print('[VisionRouter] High confidence result found, using it');
              return result;
            }
          } catch (e) {
            print('[VisionRouter] Error with classifier ${entry.key}: $e');
            // Continue to next classifier
          }
        } else {
          print('[VisionRouter] Classifier ${entry.key} not ready');
        }
      }
    }
    ...
    if (_fallbackClassifier != null) {
      print('[VisionRouter] Trying fallback classifier');
      try {
        if (await _fallbackClassifier.isReady()) {
          print('[VisionRouter] Using fallback classifier');
          final result = await _fallbackClassifier.classify(imageData);
          print('[VisionRouter] Fallback classification successful: ${result.diseaseName} (${result.confidence})');
          return result;
        } else {
          print('[VisionRouter] Fallback classifier not ready');
        }
      } catch (e) {
        print('[VisionRouter] Error with fallback classifier: $e');
      }
    } else {
      print('[VisionRouter] No fallback classifier available');
    }

    // 4. Si no hay clasificadores disponibles
    print('[VisionRouter] No available classifiers found');
...
class CacaoVisionClassifier implements VisionClassifier {
  ...
  Future<VisionResult> classify(Uint8List imageData) async {
    ...
    return VisionResult(
      ...
      metadata: {'model': name, 'processing_time_ms': 500, 'image_size': imageData.length},
    );
  }
...
class GenericVisionClassifier implements VisionClassifier {
  ...
  Future<VisionResult> classify(Uint8List imageData) async {
    ...
    return VisionResult(
      ...
      metadata: {'model': name, 'processing_time_ms': 800},
    );
  }
...
class VisionRouterFactory {
  static VisionRouter createDefault() {
    ...
    return VisionRouter(classifiers: classifiers, fallbackClassifier: fallbackClassifier);
  }
}
